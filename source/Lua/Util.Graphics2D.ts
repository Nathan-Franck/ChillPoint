...
export namespace Graphics2D {
    ...
    export function load_sheets<T extends { [key: string]: Pick<Sheet, "animations" | "sprites"> }>(renderer: External<"SDL_Renderer*">, sheet_inputs: T) {
    ...
    export function loop_animation<T extends Required<Pick<Sheet, "animations">>>(params: { time: number, sheet: T, animation: keyof T["animations"] }) {
        const sheet = params.sheet;
